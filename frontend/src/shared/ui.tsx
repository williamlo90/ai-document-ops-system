...
export function Panel({
  ...
  return (
    <section className={`ops-panel ${className}`.trim()} aria-label={ariaLabel}>
      {children}
    </section>
  )
}
...
>(function Button({ variant = 'secondary', className = '', children, ...props }, ref) {
  return (
    <button
      ref={ref}
...
export function StatusBadge({
  tone = 'neutral',
  children,
}: PropsWithChildren<{ tone?: 'neutral' | 'info' | 'success' | 'warning' | 'danger' | 'purple' }>) {
...
export function SkeletonRows({ count = 5 }: { count?: number }) {
  return (
    <div className="ops-skeleton-list" aria-label="Loading data">
