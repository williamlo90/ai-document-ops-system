...
import { AlertCircle, ArrowLeft, Check, CheckCircle2, ChevronRight, LoaderCircle, Pencil, Save, ShieldCheck, X } from 'lucide-react'
...
function DecisionPanel({ open, close, canDecide, blockers, note, setNote, select, pending, error, latestDecision, latestAudit, auditCount, status }: { open:boolean; close:()=>void; canDecide:boolean; blockers:number; note:string; setNote:(value:string)=>void; select:(kind:DecisionKind)=>void; pending:boolean; error:Error|null; latestDecision:DecisionResult['decision']|null; latestAudit:InvoiceDetailResponse['audit_events'][number]|undefined; auditCount:number; status:string }) {
  return <Panel className={`review-decision-panel ${open ? 'is-open' : ''}`} ariaLabel="Reviewer decision"><header><h2>Decision</h2><button className="ops-icon-button review-decision-close" aria-label="Close decision panel" onClick={close}><X size={18}/></button></header>{canDecide ? <><section className={blockers ? 'review-recommendation-card is-blocked' : 'review-recommendation-card'}><header><ShieldCheck size={17}/><strong>{blockers ? 'Approval blocked' : 'Ready for a decision'}</strong></header><h3>{blockers ? `${blockers} validation blocker${blockers===1?'':'s'}` : 'No validation blockers'}</h3><p>{blockers ? 'Request a correction or reject this invoice. Approval remains unavailable until validation passes.' : 'Compare the invoice data with the PDF before approving.'}</p></section><label className="review-note"><span>Decision note {blockers ? <b>*</b> : '(optional for approval)'}</span><textarea maxLength={1000} value={note} onChange={(event)=>setNote(event.target.value)} placeholder="Explain the decision for the audit trail..."/><small>{note.length} / 1000</small></label><div className="review-decision-actions"><Button variant="primary" disabled={pending||note.trim().length<3} onClick={()=>select('correction')}><AlertCircle size={16}/> Request correction</Button><Button disabled={pending||blockers>0} title={blockers?'Resolve validation blockers before approval':undefined} onClick={()=>select('approve')}><CheckCircle2 size={16}/> Approve</Button><Button variant="danger" disabled={pending||note.trim().length<3} onClick={()=>select('reject')}><X size={16}/> Reject</Button></div>{error?<p className="review-inline-error"><AlertCircle size={14}/>{error.message}</p>:null}</> : <DecisionEvidence decision={latestDecision} latestAudit={latestAudit} auditCount={auditCount} status={status} />}</Panel>
}
