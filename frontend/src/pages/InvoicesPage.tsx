  ...
  return <div className="ops-page invoices-page">
    ...
    {invoices.error ? <ErrorState message={(invoices.error as Error).message} retry={() => void invoices.refetch()} /> : <>
      ...
      <div className={`invoice-master-detail ${selected ? 'has-selection' : ''}`}>
        ...
        {selected ? <InvoiceInspector invoice={selected} detail={detail.data} loading={detail.isLoading} error={detail.error as Error | null} close={() => setFilter('invoice')} /> : null}
      </div>
    </>}
...
function InvoiceInspector({ invoice, detail, loading, error, close }: { invoice: InvoiceItem; detail?: InvoiceDetailResponse; loading: boolean; error: Error | null; close: () => void }) {
