...
import { AlertCircle, CalendarDays, ChevronRight, Clock3, FileSearch, ShieldAlert, Sparkles, X } from 'lucide-react'
...
export function ReviewQueuePage() {
  ...
  useEffect(() => {
    if (!selectedId && firstItemId && window.innerWidth >= 1180) updateParams(params, setParams, { invoice: firstItemId })
  }, [firstItemId, params, selectedId, setParams])
...
function ReviewTable({ items, selectedId, select }: { items: ReviewQueueItem[]; selectedId?: string; select: (id: string) => void }) {
  return <div className="ops-table-wrap"><table className="ops-table review-table"><thead><tr><th aria-label="Selected" /><th>Invoice</th><th>Vendor</th><th>Amount</th><th>Finding</th><th>Risk</th><th>Confidence</th><th>Age</th><th>Owner</th></tr></thead><tbody>{items.map((item) => <tr key={item.id} className={selectedId === item.id ? 'is-selected' : ''} onClick={() => select(item.id)}><td><span className="ops-radio">{selectedId === item.id ? <i /> : null}</span></td><td><button className="ops-link" onClick={(event) => { event.stopPropagation(); select(item.id) }}>{invoiceLabel(item)}</button><small className="review-source-name">{item.original_filename}</small><small className="review-mobile-vendor">{item.vendor_name || 'Vendor not detected'}</small></td><td>{item.vendor_name || '-'}</td><td>{formatMoney(item.total,item.currency)}</td><td className={item.finding ? 'review-finding' : ''}>{item.finding || 'No issue summary'}</td><td><StatusBadge tone={riskTone(item.risk)}>{item.risk[0].toUpperCase()+item.risk.slice(1)}</StatusBadge></td><td><Confidence value={item.confidence} /></td><td>{age(item.age_seconds)}</td><td><span className="ops-owner"><i>{initials(item.owner)}</i>{item.owner}</span></td></tr>)}</tbody></table></div>
}
...
function ReviewInspector({ item, detail, loading, error, close }: { item: ReviewQueueItem; detail?: InvoiceDetailResponse; loading: boolean; error: Error | null; close: () => void }) {
  ...
  return <Panel className="review-inspector" ariaLabel="Selected invoice review summary"><header><div><span>{invoiceLabel(item)}</span><StatusBadge tone={riskTone(item.risk)}>{item.risk[0].toUpperCase()+item.risk.slice(1)} risk</StatusBadge><strong>{item.vendor_name || 'Vendor not detected'} / {formatMoney(item.total,item.currency)}</strong></div><button className="ops-icon-button" aria-label="Close review summary" onClick={close}><X size={19} /></button></header>{error ? <ErrorState message={error.message} /> : loading ? <SkeletonRows count={6} /> : <div className="review-inspector-content"><section><div className="review-section-heading"><h3>Extracted data</h3><Confidence value={item.confidence} /></div><dl className="review-field-list"><Field label="Invoice number" value={data?.invoice_number} /><Field label="Vendor" value={data?.vendor_name} /><Field label="Invoice date" value={formatDate(data?.invoice_date)} /><Field label="Due date" value={formatDate(data?.due_date)} /><Field label="Total amount" value={formatMoney(data?.total,data?.currency)} /></dl></section><section className={`review-finding-card ${item.blocker_count ? 'is-blocker' : ''}`}><header><Sparkles size={17} /><strong>Finding</strong></header><h3>{item.finding || 'No validation issue summary'}</h3><p>{item.blocker_count ? 'Approval is blocked until this issue is corrected and validation passes.' : item.issue_count ? 'Review the invoice and stored evidence before deciding.' : 'No validation blocker is currently recorded.'}</p>{issues.slice(0,3).map((issue) => <div key={issue.code}><ShieldAlert size={13} />{issue.message}</div>)}</section><div className="review-preview"><PdfPreview url={`/documents/${item.id}/content`} filename={item.original_filename} /></div><section className="review-recommendation"><Sparkles size={17} /><div><strong>Recommended next step</strong><p>{item.recommended_action === 'request_correction' ? 'Request a correction because validation blockers remain.' : 'Open the invoice workspace and make a reviewer decision.'}</p></div></section><Link className="ops-button ops-button--primary review-open" to={`/review/${item.id}`}><span>Review invoice</span><ChevronRight size={16} /></Link></div>}</Panel>
}
...
function Confidence({ value }: { value: number | null }) { if(value==null)return <span className="confidence-none">Not scored</span>; const percent=Math.round(value*100); return <span className="confidence-value"><i style={{'--confidence':`${percent}%`} as React.CSSProperties} />{percent}%</span> }
