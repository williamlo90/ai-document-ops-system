...
function ReviewTable({ items, selectedId, select }: { items: ReviewQueueItem[]; selectedId?: string; select: (id: string) => void }) {
  return <div className="ops-table-wrap"><table className="ops-table review-table"><thead><tr><th aria-label="Selected" /><th>Invoice</th><th>Vendor</th><th>Amount</th><th>Finding</th><th>Risk</th><th>Confidence</th><th>Age</th><th>Owner</th></tr></thead><tbody>{items.map((item) => <tr key={item.id} className={selectedId === item.id ? 'is-selected' : ''} onClick={() => select(item.id)}><td><span className="ops-radio">{selectedId === item.id ? <i /> : null}</span></td><td><button className="ops-link" onClick={(event) => { event.stopPropagation(); select(item.id) }}>{invoiceLabel(item)}</button><small>{item.original_filename}</small></td><td>{item.vendor_name || '-'}</td><td>{formatMoney(item.total,item.currency)}</td><td className={item.finding ? 'review-finding' : ''}>{item.finding || 'No issue summary'}</td><td><StatusBadge tone={riskTone(item.risk)}>{item.risk[0].toUpperCase()+item.risk.slice(1)}</StatusBadge></td><td><Confidence value={item.confidence} /></td><td>{age(item.age_seconds)}</td><td><span className="ops-owner"><i>{initials(item.owner)}</i>{item.owner}</span></td></tr>)}</tbody></table></div>
}
