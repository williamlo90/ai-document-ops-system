...
export function PdfPreview({ url, filename }: { url: string; filename: string }) {
    ...
    const render = async () => {
      ...
      try {
        const response = await fetch(
          url,
          url.startsWith('blob:') ? undefined : { credentials: 'include' },
        )
  ...
  return (
    <Fragment>
      {fullscreen ? (
        <button
          className="pdf-fullscreen-backdrop"
          aria-label="Close fullscreen preview"
          onClick={toggleFullscreen}
        />
      ) : null}
      <aside
        className={`pdf-preview ${url ? '' : 'pdf-preview-empty'} ${fullscreen ? 'is-fullscreen' : ''}`}
      >
        ...
        {url ? (
          <>
            <div className="pdf-toolbar">
              ...
              <div className="pdf-zoom-controls">
                <button
                  ...
                  disabled={zoom <= 0.5}
                  onClick={() =>
                    setZoom((number) => Math.max(0.5, Number((number - 0.1).toFixed(1))))
                  }
                >
                  <ZoomOut size={15} />
                </button>
                ...
                <button
                  ...
                  disabled={zoom >= 2}
                  onClick={() =>
                    setZoom((number) => Math.min(2, Number((number + 0.1).toFixed(1))))
                  }
                >
                  <ZoomIn size={15} />
                </button>
                ...
                <a
                  className="outline-button pdf-open-link"
                  href={url}
                  target="_blank"
                  rel="noreferrer"
                >
            ...
            <div
              ref={topScrollRef}
              className="pdf-horizontal-scroll"
              onScroll={(event) => {
                if (stageRef.current) stageRef.current.scrollLeft = event.currentTarget.scrollLeft
              }}
            >
            ...
            >
              <canvas
                ...
                aria-label={`Page ${pageNumber} of ${filename || 'invoice'}`}
              />
              {status === 'loading' ? (
              ...
              {status === 'error' ? (
                <div className="pdf-error">
                  <FileText size={30} />
                  <strong>We could not display this PDF here.</strong>
          ...
          <div className="preview-empty">
            <FileText size={34} />
            <span>Choose a PDF to view it here.</span>
          </div>
        )}
      </aside>
    </Fragment>
