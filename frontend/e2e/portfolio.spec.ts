...
test('capture portfolio surfaces', async ({ page }) => {
  ...
  await page.route('**/*', async (route) => {
    ...
    if (pathname === `/invoices/${document.id}/workflow`) return route.fulfill({ json: { document, extraction, work_item: item, current_stage: 'needs_attention', current_owner: 'Finance Review', waiting_for: 'Human approval', next_action: 'Review evidence', attention_reason: 'Total mismatch requires confirmation.', activity: item.activity } })
