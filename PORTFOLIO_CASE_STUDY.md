...
A later diagnostic, run from a clean commit, found a different problem. The model returned a
localized amount such as `1.250,00`, but the decimal parser rejected it. The experiment log retains
the failed 19-of-20 result. Deterministic normalization fixed the parser, and a regression test
covered the failure before the diagnostic ran again.
...
The external evaluation used two licensed 25-document FATURA packs stored outside Git.

The first sealed holdout exposed a provider-availability problem: only 1 of 10 selected documents
completed both provider steps. The experiment record retains that failed result.

The second pack used 25 source layouts that had not appeared in the first pack. With the diagnostic
fixes frozen, the provider stack ran against one new sealed 10-document holdout. Mistral OCR and
OpenAI extraction completed all 10 documents.
...
- Reviewer time savings, cost savings, and error reduction have not been measured.
