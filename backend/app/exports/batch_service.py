        ...
        *,
        ...
        with self._execution_lock:
            ...
            if not all(check["state"] == "passed" for check in checks):
                raise ExportEligibilityError("Export eligibility checks failed.", tuple(checks))
            documents = [self.documents.get(document_id) for document_id in batch.document_ids]
            attempt_count = 1 + max(
                (
                    run.attempt_count
                    for run in self.repository.list_runs(context.workspace_id)
